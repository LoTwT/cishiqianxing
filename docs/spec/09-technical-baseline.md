...
- 当前运行时实现覆盖内存中的永久成长领取事务、敌人实例档案解析与确定性接触战结算候选评价；内容层另有五阶段代表路线静态合同，以及 12 个家族的基础／强化 24 份中央敌人档案。候选结果和事件明确不是提交边界，也不携带世界步、接触格位、支援成员集合或背包版本；未来世界步必须先确认这些权威上下文仍与评价时一致，再原子应用同一个候选，不得直接提交陈旧候选或重新计算战斗公式。接触战尚未接入接触锁定与取消、背包持有／替换／实际消费、六种行为执行、时序轮替、UI、动画、音频、场景、撤销或存档 DTO。现阶段只验证敌人单体档案、运行时投影、临时属性和最多两名支援的算术投影；支援组整体路线损耗仍须结合地图几何验证，不能据此认定 SYS-004 的临时属性生命周期或 SYS-010 的六种敌人行为已经完成。
- **脆弱假设：** `EnemyInstanceState` 当前故意不拥有网格坐标，也不决定敌人的生成、移除、复活或跨房间生命周期；这些职责必须由后续复合世界状态持有并以稳定 `instance_id` 关联。本类型是纯内存规则状态，不等于完整存档 DTO。未来切换基础／替代状态时还必须由世界事务明确当前耐久如何映射到所选状态最大耐久；当前四份替代档案两套最大耐久恰好相等，不能把这种内容巧合固化为规则。
