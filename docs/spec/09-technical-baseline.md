...
- 当前清单的 `schema_version` 与 `content_version` 均为2。`res://content/progression/global_progression_catalog.tres` 由该清单强类型显式引用，以9个主线章节 bundle 与4个M01—M04可选 bundle记录全局永久成长；中央校验按 bundle 内非零属性字段验证主线24项与可选6项，并逐章校验派生属性。当前边界只包含静态定义、快照查询和完整内容封印，不包含奖励领取、防重复、运行时属性应用或存档。
