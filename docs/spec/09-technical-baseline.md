...
| 静态内容 | 自定义 `.tres` Resource | 只读定义、稳定 ID、显式引用和启动校验 |
...
- Resource 只保存不可变定义，运行时不得直接修改共享 Resource。
- 24 张蓝图、24 份敌人档案、任务、经济和关卡引用必须由同一内容注册与校验职责统一管理；具体类型名在正式实现落地时确定。
- 每项内容使用稳定、语言无关的 ID。显示名称、字幕和配音通过 ID 解析，不把中文或日文正文当主键。
...
Godot 官方命令行支持 headless 脚本、资源导入和导出，但官方内建单元测试用于引擎本身，不是项目级 GDScript 测试框架。因此正式工程必须维护一个小型测试运行器，并在首个生产垂直切片前提供一条本地与持续集成共用的权威命令。命令及脚本路径只有在对应实现进入仓库并通过无头执行后才能冻结；当前最小工程不虚构尚不存在的测试入口。
...
| 持续集成平台与分支保护 | 需要跨机器自动构建或协作时 | 用户＋技术实现 |
