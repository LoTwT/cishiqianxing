...
- `TemporaryEffectSelectionCommand` 以期望 revision 和稳定堆叠ID请求选择下一场接触战的临时属性。`TemporaryEffectSelectionKernel` 只把中央蓝图20—24映射到现有五种 `ContactCombatCommand.TemporaryEffect`；首次选择产生 revision 加一的新状态候选，重复选择保持幂等，改选不同堆叠在命令未显式确认时返回 `CONFIRMATION_REQUIRED` 且状态不变，陈旧 revision 和非属性堆叠结构化拒绝。选择只保留具体来源堆叠并驱动战斗预览投影，不扣除数量、不发布战后消费事件，也不是世界提交边界；未来接触战世界事务必须复验同一背包前态并按既有战斗结果中的消费意图原子扣除或清除选择。
...
- 当前运行时实现覆盖内存中的永久成长领取事务、敌人实例档案解析、敌人世界编目与格位生命周期、便携成品方块背包解析与临时属性选择候选，以及确定性接触战结算候选评价；内容层另有五阶段代表路线静态合同，以及 12 个家族的基础／强化 24 份中央敌人档案。敌人世界快照携带 `world_step`，背包快照携带 `revision`，两者均支持完整前态比较，供后续事务识别陈旧上下文；它们本身不推进世界步，也不提交接触战候选或战后消费。接触战候选结果仍不携带接触格位、支援成员集合或背包精确前态；未来世界步必须绑定并复验全部权威前态，再原子应用同一个候选，不得直接提交陈旧候选或重新计算战斗公式。当前尚未接入七阶段世界步、接触锁定与取消、背包实际消费、常用补给与仓库、六种行为执行、时序轮替、地图 Resource 与正式摆放、UI、动画、音频、场景、撤销或存档 DTO。现阶段只验证敌人单体档案、敌人子集格位投影、便携成品堆叠与临时属性选择、临时属性和最多两名支援的算术投影；不能据此认定全实体占格组合、解除后玩家是否立即进入原敌人格、SYS-004 的战后消费、完整SYS-011或SYS-010的六种敌人行为已经完成。
