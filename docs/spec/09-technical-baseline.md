...
- `WorldStepContactKernel.revalidate()` 只接受完整的 `LOCKED` 结果，并在周期环境之后重新解析当前格网、玩家和敌人世界。玩家生命归零时以稳定优先级返回 `PLAYER_INACTIVE` 取消，目标已转为 `RESOLVED` 时返回 `ENEMY_INACTIVE` 取消；取消结果保留历史锁定快照用于诊断，但不再报告任何 actor 仍被锁定。双方仍活动时，步数、格位或敌方地址漂移都会失败关闭，而不是静默改写接触。`NO_CONTACT`、`LOCKED`、`CANCELLED` 和 `REJECTED` 均不发布状态或领域事件，不是提交边界，也不推进 `world_step`；锁定／取消结果只能由内核构造。
- 当前运行时实现覆盖内存中的永久成长领取事务、敌人实例档案解析、敌人世界编目与格位生命周期、便携成品方块背包解析与临时属性选择候选、确定性接触战候选评价、接触战原子提交子事务，以及当前空间内玩家／敌人占格组合与本步接触锁定候选；内容层另有五阶段代表路线静态合同，以及 12 个家族的基础／强化 24 份中央敌人档案。接触锁定已经从移动意图和权威占格事实推导目标、地址与主动方，也能在周期失活后取消，但尚未执行任何世界阶段或把规范支援集合交给接触战提交子事务。由于完整实体占格、阶段1—6及世界步推进尚未进入同一状态聚合，接触战子事务仍把输入视为已经到达第7阶段并保留绑定的 `world_step`。当前尚未接入完整七阶段世界步、实际位移／构造／环境／巡逻／周期阶段、支援连线拓扑重算、常用补给与仓库、六种行为执行、时序轮替、地图 Resource 与正式摆放、UI、动画、音频、场景、撤销或存档 DTO。现阶段不能据此认定锁定已经被真实阶段执行器消费、解除后玩家会立即进入原敌人格、完整SYS-004／SYS-011或SYS-010的六种敌人行为已经完成。
- **脆弱假设：** 当前接触由“移动 actor 尝试进入对方已占用格”形成，双方在战斗提交前仍停留于各自的正交相邻格；敌人解除后玩家是否立即进入其原格尚未由正式地图和世界步合同冻结。后续实现不得从当前锁定候选反推该战后位移，必须在世界步提交 Gate 中显式决定并补齐预览、占格冲突和撤销测试。
- **脆弱假设：** `EnemyInstanceState` 继续故意不拥有网格坐标；当前由 `EnemyWorldState` 以稳定 `instance_id` 持有敌人编目、格位和 `ACTIVE`／`RESOLVED` 生命周期，但仍不决定生成、复活、完整世界步或存档迁移策略。这些类型都是纯内存规则状态，不等于完整存档 DTO。未来切换基础／替代状态时还必须由世界事务明确当前耐久如何映射到所选状态最大耐久；当前四份替代档案两套最大耐久恰好相等，不能把这种内容巧合固化为规则。
...
- 当前清单的 `schema_version` 与 `content_version` 均为5，明确拒绝旧版v4清单且不建立兼容层。清单强类型显式引用 `res://content/progression/global_progression_catalog.tres`、`res://content/routes/representative_route_contract_catalog.tres` 与 `res://content/enemies/enemy_profile_catalog.tres`：前两者分别定义30个单属性永久成长奖励、9个主线组与4个M01—M04可选组，以及覆盖1–2、3–4、5–6、7–8、9章的五份阶段末代表路线合同；敌人目录定义12个家族及基础／强化共24份档案。路线合同只保存稳定引用与自身阈值；敌档的 `balance_contract_id` 只指定用来校验的阶段末平衡基准，不等于已冻结的首次登场章。未来地图校验必须禁止早于对应阶段组引用，并在真实首见章按当章主线最低状态重验。注册表为全部内容提供稳定排序列举、结构化查询和防御快照；`contract:v5` 内容封印逐项覆盖蓝图、配方、成长、路线合同、敌人家族、敌人档案及其跨域引用。内容层仍只负责静态定义、校验、快照查询和完整内容封印；运行时状态透传 v5 内容版本，不表示存档架构版本。
...
- 24 张蓝图与 24 份敌人档案已由同一内容注册、校验和内容封印职责统一管理；后续任务、经济和关卡引用接入时必须继续使用该职责，不得建立目录扫描或旁路数值表。
