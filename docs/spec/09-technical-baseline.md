...
- 当前运行时实现覆盖内存中的永久成长领取事务、敌人实例档案解析、敌人世界编目与格位生命周期，以及确定性接触战结算候选评价；内容层另有五阶段代表路线静态合同，以及 12 个家族的基础／强化 24 份中央敌人档案。敌人世界快照携带 `world_step` 并支持完整前态比较，供后续事务识别陈旧上下文；它本身不推进世界步，也不提交接触战候选。接触战候选结果仍不携带接触格位、支援成员集合或背包版本；未来世界步必须绑定并复验全部权威前态，再原子应用同一个候选，不得直接提交陈旧候选或重新计算战斗公式。当前尚未接入七阶段世界步、接触锁定与取消、背包持有／替换／实际消费、六种行为执行、时序轮替、地图 Resource 与正式摆放、UI、动画、音频、场景、撤销或存档 DTO。现阶段只验证敌人单体档案、敌人子集格位投影、临时属性和最多两名支援的算术投影；不能据此认定全实体占格组合、解除后玩家是否立即进入原敌人格、SYS-004 的临时属性生命周期或 SYS-010 的六种敌人行为已经完成。
