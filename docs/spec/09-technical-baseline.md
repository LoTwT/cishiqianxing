...
- 内存中的确定性接触战候选内核复用永久成长事务的派生入口，从 `PlayerProgressionState` 与已封印的内容注册表重建玩家权威四维和当前生命。敌方由调用方提供临时已解析状态，包含实例 ID、四维、当前耐久与护盾状态；该临时状态不是中央敌人档案，也不进入内容注册表或存档。内容启动校验会从中央档案构造这种状态并调用同一个内核；运行时从档案到敌人实例的解析仍待后续建立。
...
- 当前运行时实现覆盖内存中的永久成长领取事务与确定性接触战结算候选评价；内容层另有五阶段代表路线静态合同，以及 12 个家族的基础／强化 24 份中央敌人档案。候选结果和事件明确不是提交边界，也不携带世界步、接触格位、支援成员集合或背包版本；未来世界步必须先确认这些权威上下文仍与评价时一致，再原子应用同一个候选，不得直接提交陈旧候选或重新计算战斗公式。接触战尚未接入接触锁定与取消、敌人实例和档案解析、背包持有／替换／实际消费、六种行为执行、时序轮替、UI、动画、音频、场景、撤销或存档。现阶段只验证敌人单体档案、临时属性和最多两名支援的算术投影；支援组整体路线损耗仍须结合地图几何验证，不能据此认定 SYS-004 的临时属性生命周期或 SYS-010 的六种敌人行为已经完成。
