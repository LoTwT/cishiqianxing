...
- `WorldStepContactLock` 与 `ContactCombatTransactionCommand` 之间当前没有规则层工件互相锚定（锁定的格位、目标与移动方不是子事务前态的组成部分），把锁定几何转换为接触战命令的正确性由编排层传参保证，两层各自失败关闭。这是已知设计边界而非缺陷：在世界步执行器落地时，必须为该衔接补齐规则层校验与测试。
