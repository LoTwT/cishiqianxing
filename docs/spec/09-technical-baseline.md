...
- 内存中的确定性接触战候选内核复用永久成长事务的派生入口，从 `PlayerProgressionState` 与已封印的内容注册表重建玩家权威四维和当前生命。敌方由调用方提供临时已解析状态，包含实例 ID、四维、当前耐久与护盾状态；该状态不是敌人档案，也不进入内容注册表或存档。
...
- 当前运行时实现覆盖内存中的永久成长领取事务与确定性接触战结算候选评价；内容层另有五阶段代表路线静态合同。候选结果和事件明确不是提交边界，也不携带世界步、接触格位、支援成员集合或背包版本；未来世界步必须先确认这些权威上下文仍与评价时一致，再原子应用同一个候选，不得直接提交陈旧候选或重新计算战斗公式。接触战尚未接入接触锁定与取消、背包持有／替换／实际消费、时序轮替、UI、动画、音频、场景、撤销或存档，24 份敌人档案及其行为归属也尚未建立。现阶段只验证临时属性和支援的算术投影，不能据此认定 SYS-004 的临时属性生命周期或 SYS-010 的六种敌人行为已经完成。
