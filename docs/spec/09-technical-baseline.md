...
- 当前实现只覆盖内存中的运行时领取事务，不包含真实存档 DTO、`FileAccess`、`user://`、存档迁移、实际撤销执行、地图摆放或接触触发、UI、动画、音频、本地化、战斗、场景及九章正式内容。
...
- 当前清单的 `schema_version` 与 `content_version` 均为3，不兼容旧版v2清单。`res://content/progression/global_progression_catalog.tres` 由该清单强类型显式引用，显式定义30个单属性永久成长奖励，并以9个主线章节组和4个M01—M04可选组保存奖励ID成员关系；每个奖励必须且只能归入一组。中央校验冻结奖励字段与13组精确成员关系，再由引用推导主线24项、可选6项、逐章终值和两套最终属性。注册表提供奖励的稳定排序列举、快照查询与结构化未知ID结果；`contract:v3` 内容封印逐项覆盖奖励及成员关系。内容层仍只负责静态定义、校验、快照查询和完整内容封印；运行时领取事务消费已封印的注册表，`schema_version` 与 `content_version` 仍为3。
