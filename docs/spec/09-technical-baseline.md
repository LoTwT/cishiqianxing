...
- 当前运行时实现覆盖内存中的永久成长领取事务、敌人实例档案解析、敌人世界编目与格位生命周期、便携成品方块背包解析与临时属性选择候选、确定性接触战候选评价，以及接触战原子提交子事务；内容层另有五阶段代表路线静态合同，以及 12 个家族的基础／强化 24 份中央敌人档案。接触战子事务绑定并复验玩家、敌人世界、接触地址、规范支援成员集合与背包 revision 的全部现有权威前态，且不在提交时重新计算公式。由于玩家格位、完整实体占格、阶段1—6及世界步推进尚未进入同一状态聚合，该子事务把输入视为已经到达第7阶段，并保留绑定的 `world_step`；它不能独立判断玩家是否实际接触目标，也不能从地图拓扑重新发现支援连线。当前尚未接入完整七阶段世界步、接触锁定与周期失活取消、支援连线拓扑重算、常用补给与仓库、六种行为执行、时序轮替、地图 Resource 与正式摆放、UI、动画、音频、场景、撤销或存档 DTO。现阶段不能据此认定全实体占格组合、解除后玩家是否立即进入原敌人格、完整SYS-004／SYS-011或SYS-010的六种敌人行为已经完成。
