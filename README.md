...
当前仓库正在建立正式工程与内容生产基线，尚无可试玩的正式内容；权威规则层、内容清单 v4、五阶段代表路线合同与项目级自动化测试已经可以无头运行。权威规则层已具备内存中的永久成长领取事务与确定性接触战结算候选评价内核：玩家属性从已封印的内容注册表派生，敌方使用调用方提供的临时已解析状态；接触战覆盖伤害公式、速度与主动方决定首击、交替攻击、护盾、支援算术、临时属性投影、0 伤害阻断及非提交型候选事件。路线合同只冻结未来正式路线必须满足的边界；接触战尚未接入世界步、背包中的临时效果生命周期、24 份敌人档案、UI、动画、场景或存档。
...
- `game/src/rules/` 保存不依赖场景树、渲染、物理或输入设备的权威规则类型，包括整数格规则、永久成长的纯运行时领取事务与确定性接触战候选内核。
- `game/src/content/` 保存强类型 `Resource` 内容定义，以及不依赖场景树的 `RefCounted` 注册、校验和快照查询类型；蓝图、配方、全局永久成长与代表路线合同共用同一注册职责和内容封印。
- `game/content/` 保存正式 `.tres` 内容；`res://content/content_manifest.tres` 是唯一规范清单入口，并显式引用永久成长目录与五阶段代表路线合同目录，内容发现不扫描目录。
