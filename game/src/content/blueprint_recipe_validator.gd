class_name BlueprintRecipeValidator
extends RefCounted

# 蓝图域与配方域的唯一权威校验器。这两个域在内容清单中直接内联（没有独立的
# catalog 资源），且互为交叉引用：蓝图声明 standard_recipe_id，配方声明
# output_blueprint_id；二者还共享按蓝图序号冻结的字段期望表（类别、解锁章、
# 生命周期、机制 ID 与材料映射），因此合并为一个 validator 文件而非拆成两个。
# 四个公开静态入口对应构建期装配流水线的四个固定调用位，不合并为
# snapshot_and_validate：构建器中蓝图/配方的快照与校验和其他域的校验交错，
# 必须保持 issue 追加顺序与内容 oracle 逐字节一致。
# - snapshot_blueprints / snapshot_recipes：条目脚本检查与快照隔离。
# - validate_blueprints / validate_recipes：字段、重复、计数与交叉引用校验。
...
static func snapshot_blueprints(
...
static func validate_blueprints(
