...
static func snapshot_and_validate(
	...
	for index: int in range(exact_catalog.profiles.size()):
		...
		snapshot.combat_trait_ids.sort_custom(
...
static func _validate_families(
	...
	for family: EnemyFamilyDefinitionScript in families:
		ContentValidationSupportScript.increment_string_name_count(id_counts, family.family_id)
		ContentValidationSupportScript.increment_int_count(ordinal_counts, family.ordinal)
		ContentValidationSupportScript.increment_string_name_count(visual_counts, family.visual_family_id)
		...
		if _is_valid_source(family.source):
			ContentValidationSupportScript.increment_int_count(source_counts, family.source)
		else:
			ContentValidationSupportScript.add_issue(
				issues,
...
static func _validate_profiles(
	...
	for profile: EnemyProfileDefinitionScript in profiles:
		ContentValidationSupportScript.increment_string_name_count(id_counts, profile.profile_id)
		ContentValidationSupportScript.increment_string_name_count(visual_counts, profile.visual_binding_id)
		...
		if not ALLOWED_BEHAVIOR_IDS.has(profile.behavior_id):
			...
			ContentValidationSupportScript.increment_string_name_count(behavior_counts, profile.behavior_id)
