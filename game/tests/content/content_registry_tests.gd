...
func _builds_canonical_manifest(context: HeadlessTestContextScript) -> void:
	...
	context.expect_equal(registry.schema_version(), 4, "Schema version must be frozen at four.")
	context.expect_equal(registry.content_version(), 4, "Content version must be frozen at four.")
	...
	raw_registry._initialize_validated(999, 999, no_blueprints, no_recipes, null, null)
	context.expect_true(
		not raw_registry.is_initialized(),
